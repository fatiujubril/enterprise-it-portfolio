...
A Windows 11 Pro VM (`WIN11-USER01`), registered to Autopilot by hardware hash and with the hybrid deployment profile in the **Assigned** state, was reset to OOBE and provisioned by signing in as a single department user — **Aisha Bello** (`abello@fjlab.ca`, Finance). Nothing else was done to the device; everything below was delivered by the profile chain. The computer object created by the connector was **`LAB-6KMEnXs2vpf`**.
...
1. **OOBE — work-or-school sign-in.** The device reached the "Let's set things up for your work or school" screen — the recognition proof: the Autopilot handshake matched the hardware hash and pulled the assigned profile, skipping consumer setup entirely.
   ...
   *Note:* the sign-in showed generic Microsoft branding rather than tenant branding, because Company Branding was not configured in Entra. Cosmetic only — the profile still drove the flow — and listed in the backlog below.
...
The provisioning path completed in **under 6 minutes** end to end (excluding the break/fix). Per-phase estimates and observations: [full-flow-timing-log.md](./full-flow-timing-log.md).
...
|---|---|---|---|
| Case Study | "Your domain isn't available" at first sign-in after the ODJ reboot | The Autopilot-provisioned device booted on DHCP and received DNS from the home router (Rogers gateway at 10.0.0.1), not DC01 — so it could not resolve `corp.lab` to authenticate the domain user. A stale DHCP scope option (leftover `10.10.10.10` from the Phase 1 DC renumber) was a first red herring; the real disease is an unmanaged DHCP layer on the external vSwitch, which only surfaced because Autopilot is the first workflow to provision a device that boots on DHCP. | [breakfix-case-study.md](./breakfix-case-study.md) |

A second, minor item worth noting (not a break/fix — nothing broke): the Finance app initially "didn't install" simply because the app object had never been committed — the Store app wizard had been left at the Review + create step without clicking Create. Creating it properly, confirming it appeared in the Windows apps list, and forcing a device sync delivered it. The lesson is consistent with the connector ACL verification and the Case 02 persistence check from earlier phases: **a Review + create summary is intent, not confirmation — verify the object actually exists afterward.**

---

## Limitations

This test validated hybrid provisioning **on-network**. Hybrid Autopilot has a hard limitation for remote provisioning — the offline domain join succeeds, but the first interactive sign-in requires a reachable domain controller, which a remote device does not have. The full analysis, the three real responses (pre-logon VPN, cached credentials, or cloud-native), and the hybrid-vs-cloud-native decision framework: **[limitations-and-cloud-native.md](./limitations-and-cloud-native.md)**.

---

## Identified future work

Known gaps and improvements, listed deliberately rather than left silent:

| Item | Why it matters |
|---|---|
| **Company Branding in Entra** | The OOBE sign-in showed generic Microsoft branding rather than tenant branding. Cosmetic, but tenant-branded OOBE is part of the polished onboarding experience and is a five-minute configuration. |
| **Network redesign — internal/private vSwitch** | The root cause of this phase's break/fix. The lab is bridged to the home LAN via an external vSwitch, so the ISP router wins DHCP over DC01. Moving to an internal/private switch with DC01 as sole DHCP/DNS authority, and routing internet egress deliberately, is the correct fix. The static-IP workaround used here does not scale to real zero-touch provisioning. |
| **Re-run the test on the redesigned network** | With precise timestamps, as the representative clean baseline — and a second variant with OOBE Windows updates enabled, to quantify that cost. |
| **Conditional Access implementation** | Designed and deferred in Phase 5 pending a clean provisioning run. That run is now complete, so CA can be built against a known-good baseline — with the break-glass exclusion applied as each policy is created. |
| **Cloud-native comparison build** | Provisioning the same device via Entra-join Autopilot would make the hybrid-vs-cloud-native tradeoff concrete rather than theoretical, and would demonstrate Cloud Kerberos Trust. |

---

## Outcome

A device was provisioned end to end with zero touch: hardware hash to fully configured, domain-joined, Intune-managed, department-provisioned, least-privilege desktop — driven entirely by a single user sign-in and an attribute set once in on-premises AD, in under six minutes. The one real failure encountered was diagnosed to a root cause two layers removed from its symptom and five phases removed from its introduction, then resolved, with the proper architectural remediation identified rather than papered over. The hybrid Autopilot chain works, and the diagnostic process that got it there is the demonstrable skill.
